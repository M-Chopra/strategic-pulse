...
export default function PostCard({ post, featured = false }) {
  ...
  return (
    <div className={`${styles.cardWrapper} ${featured ? styles.featured : ''}`}>
      <Link to={`/post/${post._id}`} className={styles.card}>
        {/* Top meta row */}
      ...
      {/* Bookmark button */}
      <div className={styles.bookmarkBtn} onClick={(e) => e.stopPropagation()}>
        <BookmarkButton postId={post._id} size="large" />
      </div>
    </div>
  )
}
